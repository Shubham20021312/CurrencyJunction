...
<body class="">
  ...
  <div class="main-content">
    ...
    <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
      <div class="container-fluid">
        <!-- Brand -->
        <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="{{route('show_ticket')}}"></a>
        <!-- Form -->
    ...
    <div class="col-md-12">
        <div class="text-center mb-4">
            <p class="lead">USERS BANK DETAILS</p>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <span class="badge badge-primary">{{ $account->status }}</span>
                            @endif
                        </td>
                        <td>
                            <select class="form-control" onchange="changeStatus(this.value, {{ $account->id }})">
                                <option value="pending" {{ $account->status == 'pending' ? 'selected' : '' }}>Processing</option>
                                <option value="approved" {{ $account->status == 'approved' ? 'selected' : '' }}>Approved</option>
                                <option value="rejected" {{ $account->status == 'rejected' ? 'selected' : '' }}>Rejected</option>
                            </select>
                        </td>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/js/toastr.min.js"></script>
  <script>
    ...
    function changeStatus(status, ticketId) {
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('user_bank_details.change_status') }}",
            data: {
                status: status,
                ticketId: ticketId,
                _token: "{{ csrf_token() }}"
